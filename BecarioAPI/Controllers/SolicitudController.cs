...
namespace BecarioAPI.Controllers
{
    ...
    public class SolicitudController : Controller
    {
        ...
        public ActionResult ActualizarSolicitud(int solicitudid, [FromBody] Solicitud solicitud)
        {
            try
            {
                //validar que la solicitud exista antes de intentar actualizarla
                var solicitudExistente = _solicitudesRepository.ObtenerSolicitud(solicitudid);
                if (solicitudExistente == null)
                {
                    return NotFound($"No se encontro una solicitud con el ID {solicitudid}");
                }
                //actualizar la solicitud
                _solicitudesRepository.ActualizarSolicitud(solicitudid, solicitud);
